...
int main(int argc, char *argv[], char *envp[]) {
    ...
    //int childvar = 53;
    int parentvar = 63;
    //char *childstr = "Hello world";
    char anotherstr[50];
    // move the loop and it works fine... wat
    while (1) {
        printf("LOOP START\n");
        strcat(anotherstr, "Hey");
        int pid2 = fork();
        if (pid2 == 0) {
            printf("Child\n");
            printf("Child printing this - %p\n", &anotherstr);
            yield();
            printf("Child again\n");
            yield();
            exit();
        } else {
            printf("Parent printing this -  %p\n", &anotherstr);
            int status;
            wait(&status);
            parentvar = 70;
            int newparentvar = 78;
