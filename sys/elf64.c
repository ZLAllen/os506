...
struct task_struct *load_elf(Elf64_Ehdr *ehdr, char *argv[], char *envp[], char *name);
...
{      
    ...
    task_struct *new_task = create_new_task(true, name);
    ...
    /*
     *top to bottom: args -> envp -> envp_ptr -> null -> args_ptr -> nargs
     * */

    int nenvp = 0; //num envs; null terminated
    if(envp)
    {
        while(envp[nenvp])
        {
